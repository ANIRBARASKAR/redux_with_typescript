...
const Documents: React.FC = () => {
  ...
  const handleUpdateFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      setselectedUpdatedFile(files[0])
  ...
  const handleDownloadAll = () => {
    ...
    selectedDocuments.forEach((document) => {
      ...
      zip.file(`${document.name}`, content);
    });

    // Generate the zip file
    zip.generateAsync({ type: 'blob' }).then((blob) => {

      FileSaver.saveAs(blob, `allDocuments.zip`);
    });
  }

  const handleFolderDocuments = (documents: Document[], name: string) => {
    if (documents?.length <= 0) {
      setmessage("The folder is empty; there are no documents to be found.")
      settypeOfAlert("error")
      setalertShow(true)
      return
    }
                      ...
                      </tr>
                      {
                        folder?.document?.length > 0 && folder?.document?.map((document, index) => (
                          <tr className="folder-stage-1">
                            ...
                            <td className="text-start">
                              {
                                document.isDownloadable && hasDocumentUploadPermission && <button
                                  type="button"
                                  className="td-icon-btn"
                              ...
                              {
                                document.isEditable && hasDocumentUploadPermission && <button type="button" className="td-icon-btn" onClick={() => handleShowEditMdaol(document, folder.id)}>
                                  <i className="icon-edit"></i>
                                </button>
                              }
                            </td>
      ...
      >
        ...
        <div className="form-check form-switch">
          <input
            ...
            role="switch"
            id="enable"
            defaultChecked={isDownloadingEnabled}
